    ...
    public static class R2mapper1 extends Mapper<LongWritable, Text, IntWritable, IntWritable> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (word.length>2){//오리지널 인풋 처리
                ...
                intWritable.set(id);
                intWritable2.set(loc);
                context.write(intWritable, intWritable2);//사람 지역
            }else{ //R1(top5)처리//여기를 reducer의 setup으로 하면 좋을거같은데 어짜피 1개짜리라면
                int id;
                id=Integer.parseInt(word[1]);//이게 사람 아이디임(아까 5명 뽑힌거)
                intWritable.set(id);
...
//                top5[id]=1;//기록
    ...
    //id -1AABBDDEE <-top5인놈
    //id AAD <-아닌놈
    public static class R2SumReducer extends Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
        IntWritable intWritable = new IntWritable();
...
*/
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(WordCombined2.class);
        job.setReducerClass(R2SumReducer.class);
        job.setOutputKeyClass(IntWritable.class);
        job.setOutputValueClass(IntWritable.class);

        MultipleInputs.addInputPath(job, new Path(args[1] + "/R1"),TextInputFormat.class, R2mapper1.class);
        MultipleInputs.addInputPath(job, new Path(args[0]),TextInputFormat.class, R2mapper1.class);
        Path outputPath = new Path(args[1] + "/R2");
