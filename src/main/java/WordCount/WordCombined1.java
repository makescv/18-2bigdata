...
public class WordCombined1 extends Configured implements Tool {
    ...
    public static class WordMapper extends Mapper<LongWritable, Text, IntWritable, IntWritable> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            //String timeimsi[]=word[2].split(":");//분초 분리//simple
            //length=Integer.parseInt(timeimsi[0])*60+Integer.parseInt(timeimsi[1]);//전부 초단위로
            //loc=(int)word[3].charAt(0)-(int)'A';//알파벳 A부터 0으로

            //mydata=new CompositeWritable(id, length, loc);//성능고려해야함
            intWritable.set(id);
            //text.set(word);
    ...
    public static class SumReducer2 extends Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
        ...
        public void reduce(IntWritable key, Iterable<IntWritable> values, Context context)
            ...
            for (IntWritable value : values) {//동점자 스테이블 문제가 발생하겟다 ㅠㅠ 일단 무시
                if (count++<5)context.write(key,value);
            }
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        job1.setJarByClass(WordCombined1.class);
        ...
        job2.setJarByClass(WordCombined1.class);
        job2.setJobName("Word Invert");

        FileInputFormat.setInputPaths(job2, new Path(args[1] + "/temp1"));
        FileOutputFormat.setOutputPath(job2, new Path(args[1] + "/R1"));
    ...
    public static void main(String[] args) throws Exception {
        int exitCode = ToolRunner.run(new WordCombined1(), args);
